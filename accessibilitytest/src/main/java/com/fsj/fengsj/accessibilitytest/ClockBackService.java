...
public class ClockBackService extends AccessibilityService {
    ...
    private static final int EVENT_NOTIFICATION_TIMEOUT_MILLIS = 500;
    ...
    //fengshanjie 20151214 begin
    private static final String EVENT = "您获取到AccessibilityEvent：";
    //fengshanjie 201501214 end

    private void showEvent2String(String evemt){
        Toast.makeText(getApplicationContext(),EVENT + evemt,Toast.LENGTH_SHORT).show();
    }
    ...
    public void onServiceConnected() {
        if (isInfrastructureInitialized) {
            Log.i(LOG_TAG, "onServiceConnected():isInfrastructureInitialized=" + isInfrastructureInitialized);
            showEvent2String("onServiceConnected():isInfrastructureInitialized="+isInfrastructureInitialized);
            return;
        }
        Log.i(LOG_TAG ,"onServiceConnected():isInfrastructureInitialized="+isInfrastructureInitialized);
        showEvent2String("onServiceConnected():isInfrastructureInitialized="+isInfrastructureInitialized);
        ...
        Log.i(LOG_TAG ,"onServiceConnected():set isInfrastructureInitialized to be true" );
    ...
    public boolean onUnbind(Intent intent) {
        ...
        if (isInfrastructureInitialized) {
            Log.i(LOG_TAG ,"onUnbind():isInfrastructureInitialized="+isInfrastructureInitialized);
            // Stop the TTS service.
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        //showEvent2String(event.toString());
        ...
        if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_SPOKEN) {
            ...
            Log.i(LOG_TAG,"onAccessibilityEvent():FEEDBACK_SPOKEN");
            ...
            Log.i(LOG_TAG,"onAccessibilityEvent():FEEDBACK_AUDIBLE");
            ...
            Log.i(LOG_TAG,"onAccessibilityEvent():FEEDBACK_HAPTIC");
        } else {
           // showEvent2String("Unexpected feedback type!!");
            Log.i(LOG_TAG,"Unexpected feedback type!!!");
    ...
    public void onInterrupt() {
        ...
        if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_SPOKEN) {
            ...
            Log.i(LOG_TAG,"onInterrupt():MESSAGE_STOP_SPEAK");
        } else if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_AUDIBLE) {
            mHandler.obtainMessage(MESSAGE_STOP_PLAY_EARCON).sendToTarget();
            Log.i(LOG_TAG,"onInterrupt():MESSAGE_STOP_PLAY_EARCON");
        } else if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_HAPTIC) {
            mHandler.obtainMessage(MESSAGE_STOP_VIBRATE).sendToTarget();
            Log.i(LOG_TAG,"onInterrupt():MESSAGE_STOP_VIBRATE");
        } else {
            Log.i(LOG_TAG,"onInterrupt():Unexpected feedback!!!!");
            throw new IllegalStateException("Unexpected feedback type " + mProvidedFeedbackType);
        }
    }
